...
import com.bonc.dpi.dao.mapper.KeywordGroupMapper;
...
public class KeywordGroupService implements OperationFlowServiceI{

	@Resource
//	@Autowired
	private KeywordGroupMapper mapper;
	 ...
	 * @return
	 */
	public Page<KeywordGroup> selectList(KeywordGroup ksr, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<KeywordGroup> pageList = (Page<KeywordGroup>) this.mapper.selectList(ksr);
		return pageList;
	}
	 ...
	 * @return
	 */
	public KeywordGroup selectVoById(String id){
	 ...
	 * @return
	 */
	public KeywordGroup selectVoByPrimaryKey(KeywordGroup kgParam){
		return mapper.selectVoByPrimaryKey(kgParam);
	}
	 ...
	 * @return
	 */
	public Boolean deleteVoByPrimaryKey(KeywordGroup kgParam){
		return mapper.deleteVoByPrimaryKey(kgParam);
	}
	 ...
	 * @param vo
	 */
	public Boolean insertVo(KeywordGroup vo){
	...
	public KeywordGroup updateVo(KeywordGroup vo){
	 ...
	 * @return
	 */
	public Boolean selectCheck(KeywordGroup vo) {
	 ...
	 * @return
	 */
	public List<KeywordGroup> getGroupTypeListByGroupType(String groupType)throws Exception {
		return mapper.getGroupTypeListByGroupType(groupType);
	}
	 ...
	 * @return
	 */
	public List<KeywordGroup> getGroupTypeListByGroupName(String groupName)throws Exception {
		return mapper.getGroupTypeListByGroupName(groupName);
	}
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_keywordGroup);//excle批量导入问题数据页面反回路径
...
//		hashSetInDataBase_primaryKey.clear();//清空缓存
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			KeywordGroup vo = (KeywordGroup) of;
			...
			//长度验证
			if(groupType.length()>20){
				bl_tooLong = false;
			}
			if(groupName.length()>100){
				bl_tooLong = false;
			}
			if(groupLevel.length()>20){
				bl_tooLong = false;
			}
			...
			//没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(groupType)){
				hashSetInExcle_primaryKey.add(groupType);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			KeywordGroup voParam = new KeywordGroup();
			voParam.setGroupType(vo.getGroupType());
			KeywordGroup keywordGroup= mapper.selectVoByPrimaryKey(voParam);//根据主键查找
	...
	public Boolean insertListFalseCache(String type)throws Exception {
		...
		//type 2：更新
			...
			for (OperationFlow of : list_false_cache) {
				...
				KeywordGroup voParam = new KeywordGroup();
				voParam.setGroupType(groupType);
