...
import com.bonc.dpi.dao.mapper.ProductLabelMapper;
...
public class ProductLabelService implements OperationFlowServiceI{
	
	
	@Resource
//	@Autowired
	private ProductLabelMapper mapper;
	...
	public List<ProductLabel> selectList(ProductLabel vo) {
		List<ProductLabel> list = this.mapper.selectList(vo);
		return list;
	}
	
	public List<ProductLabel> selectLabelTree() {
		List<ProductLabel> list = this.mapper.selectListDistinct();
		for (ProductLabel productLabel : list) {
			ProductLabel p = new ProductLabel();
			p.setLabelCode1(productLabel.getLabelCode1());
			List<ProductLabel> children = this.mapper.selectList(p);
			productLabel.setChildren(children);
		}
		Collections.sort(list, new Comparator<ProductLabel>() {
			@Override
			public int compare(ProductLabel o1, ProductLabel o2) {
	 ...
	 * @return
	 */
	public Page<ProductLabel> selectList(ProductLabel vo, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<ProductLabel> pageList = (Page<ProductLabel>) this.mapper.selectList(vo);
		return pageList;
	}
	 ...
	 * @return
	 */
	public ProductLabel selectVoByLabelCode(String labelCode){
		return mapper.selectVoByLabelCode(labelCode);
	}
	 ...
	 * @return
	 */
	public ProductLabel selectVoById(String labelCode){
		return mapper.selectVoById(labelCode);
	}
	 ...
	 * @return
	 */
	public Integer checkData(ProductLabel vo){
	 ...
	 * @param vo
	 */
	public ProductLabel insertVo(ProductLabel vo){
	 ...
	 * @return 
	 */
	public ProductLabel updateVo(ProductLabel vo){
		if(StringUtils.isBlank(vo.getLabelName2Param())) {
			Boolean bll = mapper.updateContentLabelName(vo);
		}else {
	 ...
	 * @return
	 */
	public ProductLabel selectLabelName1(ProductLabel vo){
		return mapper.selectLabelName1(vo);
	}
	/**
	 * 查询一级标签
	 * @param vo
	...
	public String selectMax(ProductLabel vo){
		return mapper.selectMax(vo);
	}

	public String selectLetter(String letter){
		return mapper.selectLetter(letter);
	}
	/**
	 * 查询编码
	 * @param vo
	 * @return
	 */
	public Boolean checkLabelCode(ProductLabel vo){
		Integer i = mapper.checkLabelCode(vo);
		if(i > 0) {
			return true;
		}
		return false;
	 ...
	 * @param
	 */
	public Boolean deleteVoById(ProductLabel vo){
		Boolean bl=false;
		if(StringUtils.isBlank(vo.getLabelCode2())) {
			bl = mapper.deleteVoByContentLabelName(vo);
		}else {
			bl = mapper.deleteVoById(vo);
		}
		return bl;
	}
	 ...
	 * @return
	 */
	public Boolean selectCheck(ProductLabel vo) {
		if(StringUtils.isBlank(vo.getLabelName2Param())) {
			List<ProductLabel> list=mapper.selectCheckLabel1(vo);
			if(list!=null&&list.size()>0) {
				for(ProductLabel obj:list) {
			...
			Integer i = mapper.selectCheck(vo);
			if(i > 0) {
				return false;
			}
			return true;
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_productLabel);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			ProductLabel vo = (ProductLabel) of;
			String labelType=vo.getLableType();
			String labelName=vo.getLabelName();
			String labelCode1=vo.getLabelCode1();
			String labelName1=vo.getLabelName1();
			String labelCode2=vo.getLabelCode2();
			String labelName2=vo.getLabelName2();
			String labelCode=vo.getLabelCode();
			String joinLabelName=vo.getJoinLabelName();
			//非空校验
			if("".equals(labelType) || labelType==null){
				bl_empty = false;
			}
			if("".equals(labelName) || labelName==null){
				bl_empty = false;
			}
			if("".equals(labelCode1) || labelCode1==null){
				bl_empty = false;
			}
			if("".equals(labelName1) || labelName1==null){
				bl_empty = false;
			}
			if("".equals(labelCode) || labelCode==null){
				bl_empty = false;
			}
			if("".equals(joinLabelName) || joinLabelName==null){
				bl_empty = false;
			}
			//长度验证
			if(labelType.length()>50){
				bl_tooLong = false;
			}
			if(labelName.length()>100){
				bl_tooLong = false;
			}
			if(labelCode1.length()>50){
				bl_tooLong = false;
			}
			if(labelName1.length()>100){
				bl_tooLong = false;
			}
			if(labelCode2.length()>50){
				bl_tooLong = false;
			}
			if(labelName2.length()>100){
				bl_tooLong = false;
			}
			if(labelCode.length()>50){
				bl_tooLong = false;
			}
			if(joinLabelName.length()>100){
				bl_tooLong = false;
			}
			//产品id逻辑验证
...
//			bl_notTrue_prodId = piService.prodIdCheck(pi);//prod_id必须在产品表里
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(labelCode)){
				hashSetInExcle_primaryKey.add(labelCode);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			System.out.println(vo.getLabelCode());
			ProductLabel productLabel= mapper.selectVoByPrimaryKey(vo);//根据主键查找
