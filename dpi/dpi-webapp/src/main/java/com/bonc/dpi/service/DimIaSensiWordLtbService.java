...
import com.bonc.dpi.dao.mapper.DimIaSensiWordLtbMapper;
...
public class DimIaSensiWordLtbService implements OperationFlowServiceI {
	
	@Resource
	private DimIaSensiWordLtbMapper dimIaSensiWordLtbMapper;
	 ...
	 * @return
	 */
	public Page<DimIaSensiWordLtb> selectPageList(DimIaSensiWordLtb sensi, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<DimIaSensiWordLtb> pageList = (Page<DimIaSensiWordLtb>) this.dimIaSensiWordLtbMapper.selectList(sensi);
		return pageList;
	}	
	/**
	 * 根据id选择
	 ...
	 * @return
	 */
	public DimIaSensiWordLtb selectSensiByKey(String id) {
		DimIaSensiWordLtb sensi = this.dimIaSensiWordLtbMapper.selectSensiById(id);
		return sensi;
	}		
	/**
	 * 新增
	 ...
	 * @return
	 */
	public DimIaSensiWordLtb insertSensi(DimIaSensiWordLtb sensi) {	
		this.dimIaSensiWordLtbMapper.insertSensi(sensi);
		return sensi;
	}
	/**
	 * 更改
	 ...
	 * @return
	 */
	public DimIaSensiWordLtb updateSensi(DimIaSensiWordLtb sensi) {
		
		this.dimIaSensiWordLtbMapper.updateSensi(sensi);
		return sensi;
	}
	/**
	 * 删除敏感词
	 * @param dimIaUserAgentNoise
	 */
	public void deleteSensiById(DimIaSensiWordLtb sensi) {
		this.dimIaSensiWordLtbMapper.delete(sensi);
	}
	/**
	 * 校验
	 * @param dimIaUserAgentNoise
	 * @return
	 */
	public Boolean selectCheck(DimIaSensiWordLtb sensi) {
		Integer i = this.dimIaSensiWordLtbMapper.check(sensi);
		if(i > 0) {
			return false;
		}
		return true;
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_dimIaSensiWordLtb);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			DimIaSensiWordLtb vo = (DimIaSensiWordLtb) of;
			String sensiWord=vo.getSensiWord();
			String author = vo.getAuthor();//作者
			String opTime = vo.getOpTime();//操作时间
			
			//非空校验
			if("".equals(sensiWord) || sensiWord==null){
				bl_empty = false;
			}
			//长度验证
			if(sensiWord.length()>200){
				bl_tooLong = false;
			}	
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(sensiWord)){
				hashSetInExcle_primaryKey.add(sensiWord);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			DimIaSensiWordLtb vo = (DimIaSensiWordLtb) operationFlow;
			DimIaSensiWordLtb dimIaSensiWordLtb= dimIaSensiWordLtbMapper.selectVoByPrimaryKey(vo);//根据主键查找
...
//			vo.setId(UUID);
	...
	public Boolean insertListFalseCache(String type)throws Exception {
		...
		//type 2：更新
			...
			for (OperationFlow of : list_false_cache){
				DimIaSensiWordLtb vo = (DimIaSensiWordLtb) of;
				dimIaSensiWordLtbMapper.deleteVoByPrimaryKey(vo);
			}
	...
	public void insertListCache(List<OperationFlow> list )throws Exception {
		...
		for(int j=0;j<(list.size()/PL_NUMBERS_ONCE+1);j++){
			...
			if(list_true_insert.size()>0 && list_true_insert!=null){
				dimIaSensiWordLtbMapper.batchInsert(list_true_insert,DpiUtils.DATABASE_TYPE);
			}
		}
	}  
	...
	public OperationFlowExportReturn doExport(OperationFlow o) throws Exception {
		DimIaSensiWordLtb vo = (DimIaSensiWordLtb) o;
		if(StringUtils.isNotBlank(vo.getOpTime())) {
			String[] arr=vo.getOpTime().split("-");
			vo.setStartTime(arr[0].trim());
			vo.setEndTime(arr[1].trim());
		}
		List<DimIaSensiWordLtb> list = dimIaSensiWordLtbMapper.selectList(vo);
		OperationFlowExportReturn ofer = new OperationFlowExportReturn();
		ofer.setTxtName("dim_ia_sensi_word_ltb.txt");
		ofer.setExcelName("dim_ia_sensi_word_ltb.xlsx");
		ofer.setList(list);
		return ofer;
	}
	@Override
	public Integer exportDataNum(OperationFlow o,String type) throws Exception {
		...
		Integer num = dimIaSensiWordLtbMapper.selectDataNum(vo);
