...
function initDataGrid(){
	$('#grid-table').jqGrid({
		url : $.cxt + "/urlParameter/selectpagelist",
		datatype : "json",
		mtype : "POST",
		height : topjqGridHeight(),
		autowidth : true,
		colNames : [ '域名','产品名称','参数名','参数值','匹配规则','备注','操作人','操作时间','操作' ],
		colModel : [ 
			{name : 'host',align : 'center',index : 'host',editable : true,hidden : false},
			{name : 'prodName',align : 'center',index : 'prodName',editable : true},
			{name : 'parameterCode',align : 'center',index : 'parameterCode',editable : true},
			{name : 'parameterValue',align : 'center',index : 'parameterValue',editable : true},
...
function showUpForm(id) {
	topwindow.showWindow({
		...
		data : {id : id},
		//url
		url : $.cxt + "/urlParameter/showForm",
		bottons : [{
			title : "确认",
			fun : function() {
				//校验，参数为表单父级id
				var obj=topgetObjByObj($("#upForm .formField"));
				obj.regexpRuleParam=$("#regexpRuleParam").val();
				obj.urlExample=$("#urlExample").val();
				if(topcheckoutForm("upForm")){
					$.ajax({
						url : $.cxt + "/urlParameter/insertOrUpdateVo", 
						data : obj,
						type: "POST",
...
function deleteUp(id) {
	topshowdialog("确认删除？", function(){
		$.ajax({
			url : $.cxt + "/urlParameter/deleteVoById",
			data : {id : id},
			type: "POST",
			success : function(data) {
				//重新加载表格
...
function viewUp(id) {
	topwindow.showWindow({
		...
		data : {id : id},
		//url
		url : $.cxt + "/urlParameter/viewVoById",
	});	
}
