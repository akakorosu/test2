...
import com.bonc.dpi.dao.mapper.KeywordSearchRuleMapper;
...
public class KeywordSearchRuleService implements OperationFlowServiceI{
	
	@Autowired
	private KeywordSearchRuleMapper mapper;
	...
	public List<KeywordSearchRule> selectListSimple(KeywordSearchRule ksr) {
		List<KeywordSearchRule> list = this.mapper.selectListSimple(ksr);
		return list;
	}
	 ...
	 * @return
	 */
	public Page<KeywordSearchRule> selectList(KeywordSearchRule ksr, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<KeywordSearchRule> pageList = (Page<KeywordSearchRule>) this.mapper.selectList(ksr);
		
		for (KeywordSearchRule keywordSearchRule : pageList) {
			ProductInfo piParam = new ProductInfo();
			String prodId = keywordSearchRule.getProdid();
			piParam.setProdId(prodId);
			ProductInfo voPi = piService.selectVoByPrimaryKey(piParam);
			if(voPi!=null){
				keywordSearchRule.setProdName(DpiUtils.strDecrypt(voPi.getProdName()));
			}
			
			KeywordGroup kgParam = new KeywordGroup();
			kgParam.setGroupType(keywordSearchRule.getGroupType());
			KeywordGroup voKg = kgService.selectVoByPrimaryKey(kgParam);
			if(voKg!=null){
				keywordSearchRule.setGroupName(voKg.getGroupName());
			}
		}
	 ...
	 * @return
	 */
	public KeywordSearchRule selectVoById(String id){
	 ...
	 * @param vo
	 */
	public Boolean insertVo(KeywordSearchRule vo){
	 ...
	 * @return 
	 */
	public Boolean updateVo(KeywordSearchRule vo){
	 ...
	 * @return
	 */
	public Boolean checkRepeat(KeywordSearchRule vo) {
		Integer i_prod_id = this.mapper.prodIdCheck(vo);//验证prod_id必须在产品表里
		Integer i = this.mapper.checkRepeat(vo);//验证主键重复
		
		if(i_prod_id<1){
			return false;//如果产品表里中不存在
		}
	 ...
	 * @return
	 */
	public Boolean checkGroupType(KeywordSearchRule vo) {
	 ...
	 * @return
	 */
	public Boolean checkGroupName(KeywordSearchRule vo) {
	 ...
	 * @return
	 */
	public Boolean checkNum(KeywordSearchRule vo) {
		Integer i = this.mapper.checkRepeat(vo);
		if(i < 1) {
			return true;//没有值,即不重复
		}
		return false;//有值，即重复
	}
	 ...
	 * @return
	 */
	public List<Integer> getNumByRuleType()throws Exception {
		List<Integer> list = this.mapper.getNumByRuleType();
		return list;
	}
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_keywordSearchRule);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			KeywordSearchRule vo = (KeywordSearchRule) of;
			...
			if((!StringUtils.equals("ip", matchType))&&("".equals(host) || host==null)){
				bl_empty = false;
			}
			...
			if(ruleType.length()>20){
				bl_tooLong = false;
			}	
			if(channelName.length()>100){
				bl_tooLong = false;
			}
			if(parseRuleAndriod.length()>400){
				bl_tooLong = false;
			}
			if(getIndexAndriod.length()>20){
				bl_tooLong = false;
			}	
			if(parseRuleIos.length()>300){
				bl_tooLong = false;
			}
			if(getIndexIos.length()>20){
				bl_tooLong = false;
			}
			...
			/*
			if(parseRuleAdd.length()>500){
				bl_tooLong = false;
			}
			if(getIndexAdd.length()>10){
				bl_tooLong = false;
			}
			*/
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(host+"~"+prodid+"~"+parseRuleAndriod)){
				hashSetInExcle_primaryKey.add(host+"~"+prodid+"~"+parseRuleAndriod);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			voParam.setParseRuleAndriod(vo.getParseRuleAndriod());
//			voParam.setParseRuleIos(vo.getParseRuleIos());
			KeywordSearchRule keywordSearchRule= mapper.selectVoByPrimaryKey(voParam);//根据主键查找
...
//		}
	...
	public OperationFlowExportReturn doExport(OperationFlow o) throws Exception {
		KeywordSearchRule vo = (KeywordSearchRule) o;
		if(StringUtils.isNotBlank(vo.getOpTime())) {
			String[] arr=vo.getOpTime().split("-");
			vo.setStartTime(arr[0].trim());
			vo.setEndTime(arr[1].trim());
		}
		List<KeywordSearchRule> list = this.mapper.selectList(vo);
		OperationFlowExportReturn ofer = new OperationFlowExportReturn();
		ofer.setExcelName("dim_ia_keyword_search_rule.xlsx");
		ofer.setTxtName("dim_ia_keyword_search_rule_all.txt");
		ofer.setNewTxtName("dim_ia_keyword_search_rule.txt");
		ofer.setList(list);
		return ofer;
	}	
