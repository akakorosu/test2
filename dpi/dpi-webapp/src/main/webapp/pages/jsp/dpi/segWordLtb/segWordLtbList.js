...
function initDataGrid(){
	$('#grid-table').jqGrid({
		url : $.cxt + "/segWordLtb/selectpagelist",
		datatype : "json",
		mtype : "POST",
		height : topjqGridHeight(),
		autowidth : true,
		colNames : [ '分词','操作人','操作时间','操作'],
		colModel : [ 
			{name : 'segWord',align : 'center',index : 'segWord',editable : true,hidden : false ,formatter :addFenCi},
...
function reloadJqGrid (flag) {
	...
	$("#grid-table").jqGrid('setGridParam',{
        postData : {segWord:segWord,startTime:startTime,endTime:endTime,author:author},
        page : 1
    }).trigger("reloadGrid");
}
...
function showPiForm(id) {
	topwindow.showWindow({
		...
		data : {id : id},
		//url
		url : $.cxt + "/segWordLtb/showForm",
		bottons : [{
			title : "确认",
			fun : function() {
				if(topcheckoutForm("piForm")){
					$.ajax({
						url : $.cxt + "/segWordLtb/insertOrUpdateVo", 
...
function deletePi(id) {
	topshowdialog("确认删除？", function(){
		$.ajax({
			url : $.cxt + "/segWordLtb/deleteVoById",
			data : {id : id},
			type: "POST",
			success : function(data) {
				//重新加载表格
...
function addFenCi (cellvalue, options, cell) {
	fenCiArray.push(cellvalue);
	return cellvalue;
}
function queryAgain(){
	fenCiArray=[];
