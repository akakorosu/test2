...
import com.bonc.dpi.dao.mapper.DomainFuzzyRuleMapper;
...
public class DomainFuzzyRuleService implements OperationFlowServiceI{
	@Resource
	DomainFuzzyRuleMapper domainFuzzyRuleMapper;
	@Resource
	 ...
	 * @return
	 */
	public Page<DomainFuzzyRule> selectList(DomainFuzzyRule df, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<DomainFuzzyRule> pageList =null;
		if(StringUtils.isBlank(df.getParseRule())&&StringUtils.isBlank(df.getProdName())&&StringUtils.isBlank(df.getOpTime())&&StringUtils.isBlank(df.getAuthor())) {
			pageList = (Page<DomainFuzzyRule>) domainFuzzyRuleMapper.selectList(df);
			for (DomainFuzzyRule domainRule : pageList) {
			...
			pageList = (Page<DomainFuzzyRule>) domainFuzzyRuleMapper.selectListByPar(df);
			for (DomainFuzzyRule domainRule : pageList) {
				domainRule.setProdName(DpiUtils.strDecrypt(domainRule.getProdName()));
			}
		}
	 ...
	 * @return
	 */
	public DomainFuzzyRule selectById(DomainFuzzyRule df){
		DomainFuzzyRule dfr=domainFuzzyRuleMapper.selectById(df);
		dfr.setProdName(DpiUtils.strDecrypt(dfr.getProdName()));
		return dfr;
	}
	 ...
	 * @param
	 */
	public DomainFuzzyRule insert(DomainFuzzyRule df){
		Boolean bl = domainFuzzyRuleMapper.insert(df);
		return df;
	}
	public DomainFuzzyRule insertUuid(DomainFuzzyRule df){
		Boolean bl = domainFuzzyRuleMapper.insertUuid(df);
		return df;
	}
	public int insertDomainRule(DomainFuzzyRule df){
		 
		return  domainFuzzyRuleMapper.insertDomainRule(df);
	}
	/**
	 * 验证数据
	 * @param
	 */
	public int checkData(DomainFuzzyRule df){
		
		return domainFuzzyRuleMapper.checkData(df);
	}
    public int checkDomainRuleData(DomainFuzzyRule df){
		
		return domainFuzzyRuleMapper.checkDomainRuleData(df);
	}
    public int checkDomainFuzzyRule(DomainFuzzyRule df){
		
		return domainFuzzyRuleMapper.checkDomainFuzzyRule(df);
	}
	/**
	 * 更新url表的状态
	 * @param
	 */
	public int updateUrlStu(DomainFuzzyRule df){
		
		return domainFuzzyRuleMapper.updateUrlStu(df);
	}
	 ...
	 * @return 
	 */
	public DomainFuzzyRule update(DomainFuzzyRule df){
		Boolean bl = domainFuzzyRuleMapper.update(df);
		return df;
	}
	 ...
	 * @param
	 */
	public Boolean deleteById(DomainFuzzyRule df){
		Boolean bl = domainFuzzyRuleMapper.delete(df);
		return bl;
	}
	 ...
	 * @return
	 */
	public Boolean check(DomainFuzzyRule vo) {
		Integer i = this.domainFuzzyRuleMapper.check(vo);
		if(i > 0) {
			return false;
		}
		return true;
	 ...
	 * @return
	 */
	public DomainFuzzyRule getLabelById(DomainFuzzyRule vo) {
		DomainFuzzyRule i = this.domainFuzzyRuleMapper.getLabelById(vo);
		
		return i;
	}
	public DomainFuzzyRule getLabelByCode(DomainFuzzyRule vo) {
		DomainFuzzyRule i = this.domainFuzzyRuleMapper.getLabelByCode(vo);
	 ...
	 * @return
	 */
	public int checkProdIdAndProdName(DomainFuzzyRule vo) {
		int  i = this.domainFuzzyRuleMapper.checkProdIdAndProdName(vo);
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_domainFuzzyRule);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			DomainFuzzyRule vo = (DomainFuzzyRule) of;
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(parseRule+"~"+prodid)){
				hashSetInExcle_primaryKey.add(parseRule+"~"+prodid);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			DomainFuzzyRule voParam = new DomainFuzzyRule();
			voParam.setParseRule(vo.getParseRule());
			voParam.setProdid(vo.getProdid());
			DomainFuzzyRule domainFuzzyRule= domainFuzzyRuleMapper.selectVoByPrimaryKey(voParam);//根据主键查找
...
//			vo.setId(UUID);
	...
	public Boolean insertListFalseCache(String type)throws Exception {
		...
		if("1".equals(type)){
			...
			if(list_true_cache.size()>0 && list_true_cache!=null){
				System.out.println(list_true_cache.size());
		...
		//type 2：更新
			...
			for (OperationFlow of : list_false_cache){
				DomainFuzzyRule vo = (DomainFuzzyRule) of;
				DomainFuzzyRule voParam = new DomainFuzzyRule();//参数
				voParam.setParseRule(vo.getParseRule());
				voParam.setProdid(vo.getProdid());
				domainFuzzyRuleMapper.deleteVoByPrimaryKey(voParam);
			}
	...
	public void insertListCache(List<OperationFlow> list )throws Exception {
		...
		for(int j=0;j<(list.size()/PL_NUMBERS_ONCE+1);j++){
			...
			if(list_true_insert.size()>0 && list_true_insert!=null){
				domainFuzzyRuleMapper.insertVoPl(list_true_insert,DpiUtils.DATABASE_TYPE);
			}
		}
	}
	...
	public OperationFlowExportReturn doExport(OperationFlow o) throws Exception {
		DomainFuzzyRule vo = (DomainFuzzyRule) o;
		if(StringUtils.isNotBlank(vo.getOpTime())) {
			String[] arr=vo.getOpTime().split("-");
			vo.setStartTime(arr[0].trim());
			vo.setEndTime(arr[1].trim());
		}
		List<DomainFuzzyRule> list =null; 
		if(StringUtils.isBlank(vo.getParseRule())&&StringUtils.isBlank(vo.getProdName())&&StringUtils.isBlank(vo.getOpTime())) {
			list = domainFuzzyRuleMapper.selectList(vo);
			
		}else {
			list = domainFuzzyRuleMapper.selectListByPar(vo);
			
		}

		OperationFlowExportReturn ofer = new OperationFlowExportReturn();
		ofer.setExcelName("dim_ia_url_fuzzy_rule.xlsx");
		ofer.setTxtName("dim_ia_url_fuzzy_rule_all.txt");
		ofer.setNewTxtName("dim_ia_url_fuzzy_rule.txt");
		ofer.setList(list);
		return ofer;
	}
	@Override
	public Integer exportDataNum(OperationFlow o,String type) throws Exception {
		...
		Integer num = domainFuzzyRuleMapper.selectDataNum(vo);
