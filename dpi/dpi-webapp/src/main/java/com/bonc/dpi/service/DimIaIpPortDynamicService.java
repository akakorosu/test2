...
public class DimIaIpPortDynamicService implements OperationFlowServiceI{

	@Resource
	private DimIaIpPortDynamicMapper dimIaIpPortDynamicMapper;
	 ...
	 * @return
	 */
	public Page<DimIaIpPortDynamic> selectPageList(DimIaIpPortDynamic dim, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<DimIaIpPortDynamic> pageList =null;
		if(StringUtils.isBlank(dim.getIpPort())&&StringUtils.isBlank(dim.getProdName())&&StringUtils.isBlank(dim.getOpTime())&&StringUtils.isBlank(dim.getAuthor())) {
			pageList = (Page<DimIaIpPortDynamic>) dimIaIpPortDynamicMapper.selectList(dim);
			for (DimIaIpPortDynamic dimIaIpPortDynamic : pageList) {
				ProductInfo piParam = new ProductInfo();
				String prodId = dimIaIpPortDynamic.getProdid();
				piParam.setProdId(prodId);
				ProductInfo voPi = piService.selectVoByPrimaryKey(piParam);
				if(voPi!=null){
					dimIaIpPortDynamic.setProdName(DpiUtils.strDecrypt(voPi.getProdName()));
				}
			}
		}else {
			pageList = (Page<DimIaIpPortDynamic>) dimIaIpPortDynamicMapper.selectListByPar(dim);
			for (DimIaIpPortDynamic domainRule : pageList) {
	 ...
	 * @return
	 */
	public DimIaIpPortDynamic selectById(String id) {
		DimIaIpPortDynamic dimIaIpPortDynamic = this.dimIaIpPortDynamicMapper.selectById(id);
		dimIaIpPortDynamic.setProdName(DpiUtils.strDecrypt(dimIaIpPortDynamic.getProdName()));
	 ...
	 * @return
	 */
	public DimIaIpPortDynamic insert(DimIaIpPortDynamic dimIaIpPortDynamic){
		this.dimIaIpPortDynamicMapper.insert(dimIaIpPortDynamic);
		return dimIaIpPortDynamic;
	}
	/**
	 * 更新
	 ...
	 * @return
	 */
	public DimIaIpPortDynamic update(DimIaIpPortDynamic dimIaIpPortDynamic) {
		this.dimIaIpPortDynamicMapper.update(dimIaIpPortDynamic);
		return dimIaIpPortDynamic;
	}	
	/**
	 * 删除
	 * @param bl
	 */
	public void delete(DimIaIpPortDynamic bl) {
		this.dimIaIpPortDynamicMapper.delete(bl);
	}
	...
	public List<ProductInfo> getProdIdListByProdId(String prodId)throws Exception {
		DimIaIpPortDynamic ip=new DimIaIpPortDynamic();
		ip.setProdid(prodId);
		return dimIaIpPortDynamicMapper.getProdIdListByProdId(ip);
	}
	...
	public List<ProductInfo> getProdIdListByProdName(String prodName)throws Exception {
		DimIaIpPortDynamic ip=new DimIaIpPortDynamic();
		ip.setProdName(prodName);
		return dimIaIpPortDynamicMapper.getProdIdListByProdName(ip);
	}
	 ...
	 * @return
	 */
	public Boolean check(DimIaIpPortDynamic dimIaIpPortDynamic) {
		Integer i = this.dimIaIpPortDynamicMapper.check(dimIaIpPortDynamic);
		if(i > 0) {
			return false;
		}
		return true;
	 ...
	 * @return
	 */
	public Boolean checkId(DimIaIpPortDynamic dimIaIpPortDynamic) {
		dimIaIpPortDynamic.setProdName(DpiUtils.strEncrypt(dimIaIpPortDynamic.getProdName()));
		Integer i = this.dimIaIpPortDynamicMapper.checkId(dimIaIpPortDynamic);
		if(i > 0) {
			return true;
		}
		return false;
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_dimIaIpPortDynamic);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			String ipPort = vo.getIpPort();//域名host
			String ip = vo.getIp();//ip
			String port = vo.getPort();//port
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(ipPort)){
				hashSetInExcle_primaryKey.add(ipPort);
			}
			else{
...
//			vo.setId(UUID);
	...
	public Boolean insertListFalseCache(String type)throws Exception {
		...
		//type 2：更新
			...
			for (OperationFlow of : list_false_cache){
				DimIaIpPortDynamic vo = (DimIaIpPortDynamic) of;
				DimIaIpPortDynamic voParam = new DimIaIpPortDynamic();//参数
				voParam.setIpPort(vo.getIpPort());
				dimIaIpPortDynamicMapper.deleteVoByPrimaryKey(voParam);
			}
	...
	public void insertListCache(List<OperationFlow> list )throws Exception {
		...
		for(int j=0;j<(list.size()/PL_NUMBERS_ONCE+1);j++){
			...
			if(list_true_insert.size()>0 && list_true_insert!=null){
				dimIaIpPortDynamicMapper.insertVoPl(list_true_insert);
			}
		}
	}
	...
	public OperationFlowExportReturn doExport(OperationFlow o) throws Exception {
		DimIaIpPortDynamic vo = (DimIaIpPortDynamic) o;
		if(StringUtils.isNotBlank(vo.getOpTime())) {
			String[] arr=vo.getOpTime().split("-");
			vo.setStartTime(arr[0].trim());
			vo.setEndTime(arr[1].trim());
		}
		List<DimIaIpPortDynamic> list =null;
		if(StringUtils.isBlank(vo.getIpPort())&&StringUtils.isBlank(vo.getProdName())&&StringUtils.isBlank(vo.getOpTime())) {
			list =dimIaIpPortDynamicMapper.selectList(vo);
		}else {
			list = dimIaIpPortDynamicMapper.selectListByPar(vo);
		}
		OperationFlowExportReturn ofer = new OperationFlowExportReturn();
		ofer.setExcelName("dim_ia_ip_port_dynamic.xlsx");
		ofer.setTxtName("dim_ia_ip_port_dynamic_all.txt");
		ofer.setNewTxtName("dim_ia_ip_port_dynamic.txt");
		ofer.setList(list);
		return ofer;
	}
	/**
	...
	public Integer exportDataNum(OperationFlow o,String type) throws Exception {
		...
		Integer num = dimIaIpPortDynamicMapper.selectDataNum(vo);
