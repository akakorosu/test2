...
import com.bonc.dpi.dao.mapper.DimIaUserAgentKeyMapper;
...
public class DimIaUserAgentKeyService  implements OperationFlowServiceI{

	@Resource
	private DimIaUserAgentKeyMapper dimIaUserAgentKeyMapper;
	@Resource
	 ...
	 * @return
	 */
	public Page<DimIaUserAgentKey> selectPageList(DimIaUserAgentKey dimIaUserAgentKey, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<DimIaUserAgentKey> pageList = null;
		if(StringUtils.isBlank(dimIaUserAgentKey.getUaKey())&&StringUtils.isBlank(dimIaUserAgentKey.getProdName())&&StringUtils.isBlank(dimIaUserAgentKey.getOpTime())&&StringUtils.isBlank(dimIaUserAgentKey.getAuthor())) {
			pageList = (Page<DimIaUserAgentKey>) dimIaUserAgentKeyMapper.selectList(dimIaUserAgentKey);
			for (DimIaUserAgentKey dim : pageList) {
				ProductInfo piParam = new ProductInfo();
				String prodId = dim.getProdId();
				String prodid = dim.getProdIdParam();
				if(prodId!=null){
			...
			pageList = (Page<DimIaUserAgentKey>) dimIaUserAgentKeyMapper.selectListByPar(dimIaUserAgentKey);
			for (DimIaUserAgentKey dim : pageList) {
	 ...
	 * @return
	 */
	public DimIaUserAgentKey selectKeyById(String id) {
		DimIaUserAgentKey dimIaUserAgentKey = this.dimIaUserAgentKeyMapper.selectKeyById(id);
	 ...
	 * @return
	 */
	public DimIaUserAgentKey insertKey(DimIaUserAgentKey dimIaUserAgentKey){
		this.dimIaUserAgentKeyMapper.insertKey(dimIaUserAgentKey);
		return dimIaUserAgentKey;
	}
	/**
	 * 更新
	 ...
	 * @return
	 */
	public int updateUa(DimIaUserAgentKey dimIaUserAgentKey){
		
		return this.dimIaUserAgentKeyMapper.updateUa(dimIaUserAgentKey);
	}
	/**
	 * 更新
	 * @param dimIaUserAgentKey
	 * @return
	 */
	public DimIaUserAgentKey updateKey(DimIaUserAgentKey dimIaUserAgentKey) {
		this.dimIaUserAgentKeyMapper.updateKey(dimIaUserAgentKey);
		return dimIaUserAgentKey;
	}	
	/**
	 * 删除
	 * @param bl
	 */
	public void deleteUaKeyById(DimIaUserAgentKey bl) {
		this.dimIaUserAgentKeyMapper.delete(bl);
	}
	/**
	 * 校验
	 * @param dimIaUserAgentKey
	 * @return
	 */
	public Boolean check(DimIaUserAgentKey dimIaUserAgentKey) {
		Integer i = this.dimIaUserAgentKeyMapper.check(dimIaUserAgentKey);
		if(i > 0) {
			return false;
		}
		return true;
	 ...
	 * @return
	 */
	public Boolean checkId(DimIaUserAgentKey dimIaUserAgentKey) {
		dimIaUserAgentKey.setProdName(DpiUtils.strEncrypt(dimIaUserAgentKey.getProdName()));
		Integer i = this.dimIaUserAgentKeyMapper.checkId(dimIaUserAgentKey);
		if(i > 0) {
			return true;
		}
		return false;
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_dimIaUserAgentKey);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			DimIaUserAgentKey vo = (DimIaUserAgentKey) of;
			String uaKey = vo.getUaKey();
			String prodId = vo.getProdId();//产品ID PROD_ID
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(uaKey+"~"+prodId)){
				hashSetInExcle_primaryKey.add(uaKey+"~"+prodId);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			DimIaUserAgentKey vo = (DimIaUserAgentKey) operationFlow;
			DimIaUserAgentKey dimIaUserAgentKey= dimIaUserAgentKeyMapper.selectVoByPrimaryKey(vo);//根据主键查找
...
//			vo.setId(UUID);
	...
	public Boolean insertListFalseCache(String type)throws Exception {
		...
		//type 2：更新
			...
			for (OperationFlow of : list_false_cache){
				DimIaUserAgentKey vo = (DimIaUserAgentKey) of;
				dimIaUserAgentKeyMapper.deleteVoByPrimaryKey(vo);
			}
	...
	public void insertListCache(List<OperationFlow> list )throws Exception {
		...
		for(int j=0;j<(list.size()/PL_NUMBERS_ONCE+1);j++){
			...
			if(list_true_insert.size()>0 && list_true_insert!=null){
				dimIaUserAgentKeyMapper.batchInsert(list_true_insert,DpiUtils.DATABASE_TYPE);
			}
		}
	}
	...
	public OperationFlowExportReturn doExport(OperationFlow o) throws Exception {
		DimIaUserAgentKey dimIaUserAgentKey = (DimIaUserAgentKey) o;
		if(StringUtils.isNotBlank(dimIaUserAgentKey.getOpTime())) {
			String[] arr=dimIaUserAgentKey.getOpTime().split("-");
			dimIaUserAgentKey.setStartTime(arr[0].trim());
			dimIaUserAgentKey.setEndTime(arr[1].trim());
		}
		List<DimIaUserAgentKey> list =null;
		if(StringUtils.isBlank(dimIaUserAgentKey.getUaKey())&&StringUtils.isBlank(dimIaUserAgentKey.getProdName())&&StringUtils.isBlank(dimIaUserAgentKey.getOpTime())) {
			list = dimIaUserAgentKeyMapper.selectList(dimIaUserAgentKey);
		}else {
			list = dimIaUserAgentKeyMapper.selectListByPar(dimIaUserAgentKey);
		}
		
		OperationFlowExportReturn ofer = new OperationFlowExportReturn();
		ofer.setExcelName("dim_ia_useragent_key.xlsx");
		ofer.setTxtName("dim_ia_useragent_key_all.txt");
		ofer.setNewTxtName("dim_ia_useragent_key.txt");
		ofer.setList(list);
		return ofer;
	}
	@Override
	public Integer exportDataNum(OperationFlow o,String type) throws Exception {
		DimIaUserAgentKey vo = (DimIaUserAgentKey) o;
		if(StringUtils.isNotBlank(vo.getOpTime())) {
			String[] arr=vo.getOpTime().split("-");
			vo.setStartTime(arr[0].trim());
			vo.setEndTime(arr[1].trim());
		}
		Integer num = dimIaUserAgentKeyMapper.selectDataNum(vo);
