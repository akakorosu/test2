...
import com.bonc.dpi.dao.mapper.SegWordLtbMapper;
...
public class SegWordLtbService  implements OperationFlowServiceI{
	
	
	@Resource
//	@Autowired
	private SegWordLtbMapper mapper;
	 ...
	 * @return
	 */
	public Page<SegWordLtb> selectList(SegWordLtb vo, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<SegWordLtb> pageList = (Page<SegWordLtb>) this.mapper.selectList(vo);
		return pageList;
	}
	 ...
	 * @return
	 */
	public SegWordLtb selectVoById(String id){
	 ...
	 * @param vo
	 */
	public SegWordLtb insertVo(SegWordLtb vo){
	 ...
	 * @return 
	 */
	public SegWordLtb updateVo(SegWordLtb vo){
	 ...
	 * @return
	 */
	public Boolean selectCheck(SegWordLtb vo) {
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_segWord);//excle批量导入问题数据页面反回路径
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			//主键没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(segWord)){
				hashSetInExcle_primaryKey.add(segWord);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			SegWordLtb dimIaIpPortDynamic= mapper.selectVoByPrimaryKey(vo);//根据主键查找
