var searchObj={};
$(function() {
	//根据产品名称查询产品ID
	$("#searchPiListInsert").click(function(){
		if($.isEmptyObject(searchObj)){
			messageAlert("未查询到编码");
			return;
		}
		if(searchObj!=null &&searchObj!=undefined &&searchObj!=''){
			
			$("#contentLabelCodeInsert").val(searchObj.labelCode);
			//$("#prodName").val(searchObj.labelName);
		}
		
		//getProdIdList();
	});
	initProdName();
	initCatContentType();
	/*$(".chosen-select").change(function(){
		
		var prodid=$(".chosen-select").find("option:selected").val();
		if(prodid){
			var text=$(".chosen-select").find("option:selected").text();
			$("#prodId").val(prodid);
			$("#prodName").val(text);
		}
		
		
	});*/
	$("#tableTypeInsert").chosen({
		disable_search : true
	}).change(function(option, checked){
		initProdName();
    });
	
	
});

function initProdId(){
	var options='';
	//alert($("#prodId").val());
	var id=$("#prodId").val();
	$.ajax({
		url : $.cxt + "/unidentifiedUrl/getAllProdId", 
		data : {},
		type: "POST",
		success : function(data) {
			var data=JSON.parse(data);
			list=data.data;
			
			$("#selectName").empty();
			options+="<option value=''>--请选择产品--</option>";
			var top=list.length>=50 ? top=50: top=list.length;
			for(var i=0;i<top;i++){
				if(id==list[i].PROD_ID){
					options+="<option selected='selected' value='"+list[i].PROD_ID+"'>"+list[i].PROD_NAME+"</option>";
				}else{
					options+="<option value='"+list[i].PROD_ID+"'>"+list[i].PROD_NAME+"</option>";
				}
				
			}
			//options='<option>产品ID</option>'+options;
			$("#selectName").append(options);
			

		
		}
	});
}
function initProdName(){
	var options='';
	//alert($("#prodId").val());
	var type=$("#tableTypeInsert").val();
	$.ajax({
		url : $.cxt + "/substanceLabel/initProdName", 
		data : {tableName:type},
		type: "POST",
		success : function(data) {
		
			var data=JSON.parse(data);
			list=data.data;
			
			$("#prodNameInsert").empty();
			//options+="<option value=''>--请选择产品--</option>";
			//var top=list.length>=50 ? top=50: top=list.length;
			if(list.length=='0'){
				$("#prodIdInsert").val('');
				options='';
				options+="<option value=''>无产品</option>";
			}
			for(var i=0;i<list.length;i++){
				if(i==0){
					$("#prodIdInsert").val(list[i].prodId);
				}
				options+="<option value='"+list[i].prodId+"'>"+list[i].prodName+"</option>";
				
				
			}
			
			$("#prodNameInsert").append(options).trigger("chosen:updated");
			$("#prodNameInsert").chosen({
				disable_search : true
			}).change(function(option, checked){
		    	var prodNameInsert=$("#prodNameInsert option:selected").text();
		    	//alert(prodNameInsert);
		    	$("#prodIdInsert").val($("#prodNameInsert").val());
		    	
		    });
			
			

		
		}
	});
}
function initCatContentType(){
	var options='';
	
	
	$.ajax({
		url : $.cxt + "/substanceLabel/initCatContentType", 
		data : {},
		type: "POST",
		success : function(data) {
		
			var data=JSON.parse(data);
			list=data.data;
			
			$("#catContentType").empty();
			//options+="<option value=''>--请选择产品--</option>";
			//var top=list.length>=50 ? top=50: top=list.length;
			if(list.length=='0'){
				
				options='';
				options+="<option value=''>无</option>";
			}
			for(var i=0;i<list.length;i++){
				
				options+="<option value='"+list[i].contentLabelCode2+"'>"+list[i].contentLabelName2+"</option>";
				
				
			}
			
			$("#catContentType").append(options).trigger("chosen:updated");
			$("#catContentType").chosen({
				disable_search : true
			}).change(function(option, checked){
		    	/*var prodNameInsert=$("#catContentType option:selected").text();
		    	//alert(prodNameInsert);
		    	$("#prodIdInsert").val($("#prodNameInsert").val());*/
		    	
		    });
			
			

		
		}
	});
}
function getProdIdList(){
	var prodName=$("#name").val();
	prodName=$.trim(prodName);
	if(prodName){
	$.ajax({
		url : $.cxt + "/unidentifiedUrl/getProId", 
		data : {prodName:prodName},
		type: "POST",
		success : function(data) {
			var data=JSON.parse(data);
			list=data.data;
			var options="";
			
			$("#selectName").empty();
			options+="<option value=''>--请选择产品--</option>";
			for(var i=0;i<list.length;i++){                		                			                	
				options+="<option value='"+list[i].PROD_ID+"'>"+list[i].PROD_NAME+"</option>";
			}
			//options='<option>产品ID</option>'+options;
			$("#selectName").append(options);

		
		}
	});
	}
}
$("#likeSearch").keydown(function(){//查询输入框键盘查询事件
	setTimeout(function(){
		
		inputFastSearchFun();
		
	},10)
});
var inputFastSearchFun = function() {
	
	var v = $("#likeSearch").val();
	
	if(v == "") {//输入值为空则将下拉框清空，并隐藏
		$("#divFastSearch").html("");
		$("#divFastSearch").hide();
		return false;
	}
	var data = {prodName : v};
	var contentLabelName1=$("#tableTypeInsert option:selected").text();
	data.contentLabelName1=contentLabelName1;
	$.ajax({//模糊查询标签
		url : $.cxt + "/substanceLabel/comLikeSearch", 
		dataType : "json",
		data : data,
		type : "post",
		loading : 0,//去掉加载遮盖层
		success : function(data) {
		
			var obj=JSON.parse(data);
			var list=obj.data;
			var html = "";//拼接下拉框html
			var nu=list.length;
			for(var i = 0; i < list.length; i ++) {
				
				var o = list[i];
				html += "<div>";
				html += getSetLabelCloseWindowHtml(o, "1");
				
				html += "</div>";
			}
			$("#divFastSearch").html(html);
			if(html == "") {//html为空则隐藏下拉框，否则显示
				$("#divFastSearch").hide();	
			} else {
				$("#divFastSearch").show();	
			}
		}
	});
}
var getSetLabelCloseWindowHtml = function(obj, level) {
	var labelCode = obj.contentLabelCode;
	var labelName = obj.likeSearchString;
	
	var o = {
		labelCode : labelCode,
		labelName : labelName,
		level : level,
	}
	var os = JSON.stringify(o);
	var html = "<a onclick='setLabelCloseWindow(" + os + ")' href='#' labelCode='" + labelCode + "'>" + labelName + "</a>"
	return html;
}

var setLabelCloseWindow = function(cs) {
	searchObj=cs;
	$("#likeSearch").val(cs.labelName);
	$("#divFastSearch").hide();
}
function messageAlert(message){
	bootbox.dialog({
        message: "<span style=\"color:#000\">"+message+"</span>",
        title: "消息提示",
        buttons: {
            OK: {
                label: "确定",
                className: "btn-success",
            }
        }
    });
}


 



