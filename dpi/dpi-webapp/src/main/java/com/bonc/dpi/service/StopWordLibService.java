...
import com.bonc.dpi.dao.mapper.StopWordLibMapper;
...
public class StopWordLibService implements OperationFlowServiceI{

	
	@Resource
	private StopWordLibMapper mapper;
	 ...
	 * @return
	 */
	public Page<StopWordLib> selectList(StopWordLib vo, Integer page, Integer row) {
		PageHelper.startPage(page, row);
		Page<StopWordLib> pageList = (Page<StopWordLib>) this.mapper.selectList(vo);
		return pageList;
	}
	 ...
	 * @return
	 */
	public StopWordLib selectVoById(String id){
	...
	public StopWordLib insertVo(StopWordLib vo){
	 ...
	 */
	@SuppressWarnings("unused")
	public StopWordLib updateVo(StopWordLib vo){
	 ...
	 * @return
	 */
	public Boolean selectCheck(StopWordLib vo) {
	...
	public OperationFlowImportReturn doImport(List<OperationFlow> list,String type) throws Exception {
		...
		ofir.setActionUrl(DpiUtils.url_falseDataViewFromExcle_stopWordLib);//excle批量导入问题数据页面反回路径
...
//		hashSetInDataBase_primaryKey.clear();//清空缓存
	...
	public void dataValidityInExcle(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow of : list) {
			...
			//没有重复则插入缓存中
			if(!hashSetInExcle_primaryKey.contains(stopWord)){
				hashSetInExcle_primaryKey.add(stopWord);
			}
			else{
	...
	public Map<String,List<OperationFlow>> filterList_CompareDataBase(List<OperationFlow> list) throws Exception {
		...
		for (OperationFlow operationFlow : list) {
			...
			String stopWord=vo.getStopWord();//stop_word,分类关键词
			StopWordLib voParam = new StopWordLib();
			voParam.setStopWord(stopWord);
			StopWordLib stopWordLib= mapper.selectVoByPrimaryKey(voParam);//根据主键查找
	...
	public Boolean insertListFalseCache(String type)throws Exception {
		...
		//type 2：更新
			...
			for (OperationFlow of : list_false_cache) {
				StopWordLib vo = (StopWordLib) of;
				StopWordLib voParam = new StopWordLib();
				voParam.setStopWord(vo.getStopWord());;
