...
<div id="prodIdList">
	<table style="width: 750px; overflow-x:hidden; " class="table table-striped table-hover table-bordered">
    	...
    	<tr>
    		<td>
		     	<label>&nbsp;URL：</label>
		    	<input id ="urlCheck" name = "urlCheck" value = "" style="width:650px;height:30px;" />
    		</td>
    	</tr>
    	<tr>
    		<td>
...
<script>
	...
	function testRule(){
		...
		$.ajax({
			url : $.cxt + "/positionRule/checkRule",
			data:{
	...
	function isUsable(value){
		...
		topshowdialog(titleName, function(){
			...
			if(value==0){
				...
				//回显到经度,纬度,系统规则
				if(type==1){
					$("#longitudeRule").val($("#rule").val());
					$("#getIndexLong").val($("#numLocation").val());
				}else if(type==2){
					$("#latitudeRule").val($("#rule").val());
					$("#getIndexLati").val($("#numLocation").val());
				}else if(type==3){
					$("#systemRule").val($("#rule").val());
					$("#getIndexSys").val($("#numLocation").val());
				}
