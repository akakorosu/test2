	...
	<div id="myTabContent" class="tab-content">
		<div class="tab-pane fade in active" id="tab1">
		    ...
		    <table style="width: 850px; overflow-x:hidden; " class="table table-striped table-hover table-bordered">
		        <thead>
		            <tr><th title="excle行号" style="color: red;width: 90px;">excle行号</th><th title="主键">分组编码</th><th>分组名称</th></tr>
		        </thead>
		        <tbody>
		        	<c:forEach items="${requestScope.invalidDataInExcle_repeatPrimaryKey}" var="vo" varStatus="id"><tr><td>${vo.rowNum+1}</td><td>${vo.groupType}</td><td>${vo.groupName}</td></tr></c:forEach>
				</tbody>
		    </table>
	   	</div>
		...
		<div class="tab-pane fade" id="tab2">
		    ...
		    <table style="width: 850px; overflow-x:hidden; " class="table table-striped table-hover table-bordered">
		        ...
		        <tbody>
		        	<c:forEach items="${requestScope.invalidDataInExcle_empty}" var="vo" varStatus="id"><tr><td>${vo.rowNum+1}</td><td>${vo.groupType}</td><td>${vo.groupName}</td></tr></c:forEach>
				</tbody>
		    </table>
		</div>
		...
		<div class="tab-pane fade" id="tab3">
		    ...
		    <table style="width: 850px; overflow-x:hidden; " class="table table-striped table-hover table-bordered">
		        ...
		        <tbody>
		        	<c:forEach items="${requestScope.invalidDataInExcle_tooLong}" var="vo" varStatus="id"><tr><td>${vo.rowNum+1}</td><td>${vo.groupType}</td><td>${vo.groupName}</td></tr></c:forEach>
				</tbody>
		    </table>
		</div>
		...
		<div class="tab-pane fade" id="tab5">
			...
			<table style="width: 850px; overflow-x:hidden; " class="table table-striped table-hover table-bordered">
			     <tr align="center">
			     	<td>
			     		<button style="width: 300px;" class="btn btn-warning"  onclick="falseDataFromExcleUpdate('1','/keywordGroup/falseDataFromExcleUpdate')">舍&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;弃</button>
			     		<button style="width: 300px;" class="btn btn-success"  onclick="falseDataFromExcleUpdate('2','/keywordGroup/falseDataFromExcleUpdate')">更&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;新</button>
					</td>
			     </tr>	
		     </table>
		    ...
		    <table style="width: 850px; overflow-x:hidden; " class="table table-striped table-hover table-bordered">
		        ...
		        <tbody>
		        	<c:forEach items="${requestScope.list_false_cache}" var="vo" varStatus="id"><tr><td>${vo.rowNum+1}</td><td>${vo.groupType}</td><td>${vo.groupName}</td></tr></c:forEach>
				</tbody>
		    </table>
		</div>						
	</div>
